...
## Lancer le serveur (PowerShell / Windows)

1. Depuis le dossier du projet backend (recommandé) :
...
Avant de démarrer, définissez au minimum les variables d'environnement suivantes (exemples) :
...
- `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` — accès S3 (optionnel en dev si vous utilisez une solution locale)
- `S3_ENDPOINT` — endpoint custom (minio, scaleway, etc.), optionnel
- `FRONTEND_URL` — origine autorisée pour CORS (optionnel)
- `PORT` — port d'écoute (optionnel)
